...
if [[ ${CRON_SCHEDULE} != '0' ]]; then
    cat <<EOF >/mysql-backup-env
MYSQL_PORT=${MYSQL_PORT}
MYSQL_PASSWD=${MYSQL_PASSWD}
MYSQL_HOST=${MYSQL_HOST}
MYSQL_USER=${MYSQL_USER}
MYSQL_PROTO=${MYSQL_PROTO}
...
MAILTO=${MAILTO}
MSMTPRC=${MSMTPRC}
MSMTP_ACCOUNT_NAME=${MSMTP_ACCOUNT_NAME}
EOF
    touch /backup.log
    tail -F /backup.log &
    echo "${CRON_SCHEDULE} /bin/bash /mysql_backup.sh >> /backup.log 2>&1" > /crontab
    crontab /crontab
    exec cron -f -L 8
fi
